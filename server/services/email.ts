...
function getFromEmail(): string {
  return process.env.RESEND_FROM_EMAIL ?? 'info@eeuwighart.nl';
}
...
export async function sendOrderConfirmationToCustomer(data: OrderConfirmationData): Promise<void> {
  const resend = getResendClient();
  const from = getFromEmail();
    ...
    html: `
      <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #2c2c2c;">
        ...
        <p style="color: #555; font-size: 14px; line-height: 1.7;">
          ...
          <a href="mailto:info@eeuwighart.nl" style="color: #8b7355;">info@eeuwighart.nl</a>.
